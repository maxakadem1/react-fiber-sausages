...
import { useThree, useTransition } from '@react-three/fiber'
...
export default function Noodles(props) {
  ...
  const transition = useTransition(location, (location) => location.pathname, {
    from: { position: [0, 0, 0] },
    enter: { position: [0, 0, 10] },
    leave: { position: [0, 0, -10] }
  })

  return transition((style, location) => <animated.group style={style}>{noodles}</animated.group>)
}
